...
plot4.data <-read.table(sourceData, sep = ";", header = TRUE, stringsAsFactors = FALSE, dec = ".")


# subsetting data witin dates 2007-02-01 and 2007-02-0

subsetplot4.data <- plot4.data[plot4.data$Date %in% c("1/2/2007","2/2/2007"),]

# checking varriable class and convert to numeric
names(subsetplot4.data)
str(subsetplot4.data)
globalActivepower <- as.numeric(subsetplot4.data$Global_active_power)

# covert variable to required date and time format
Date.Time <- strptime(paste(subsetplot4.data$Date, subsetplot4.data$Time, sep = " "), "%d/%m/%Y %H:%M:%S")
