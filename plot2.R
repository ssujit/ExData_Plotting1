...
plot2.data <-read.table(sourceData, sep = ";", header = TRUE, stringsAsFactors = FALSE, dec = ".")


# subsetting data witin dates 2007-02-01 and 2007-02-0

subsetplot2.data <- plot2.data[plot2.data$Date %in% c("1/2/2007","2/2/2007"),]

# checking varriable class and convert to numeric
names(subsetplot2.data)
str(subsetplot2.data)
globalActivepower <- as.numeric(subsetplot2.data$Global_active_power)

# covert data to required date and time format
DateTime <- strptime(paste(subsetplot2.data$Date, subsetplot2.data$Time, sep = " "), "%d/%m/%Y %H:%M:%S")
