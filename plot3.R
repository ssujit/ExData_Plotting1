...
plot3.data <-read.table(sourceData, sep = ";", header = TRUE, stringsAsFactors = FALSE, dec = ".")


# subsetting data witin dates 2007-02-01 and 2007-02-0

subsetplot3.data <- plot3.data[plot3.data$Date %in% c("1/2/2007","2/2/2007"),]

# checking varriable class and convert to numeric
names(subsetplot3.data)
str(subsetplot3.data)
globalActivepower <- as.numeric(subsetplot3.data$Global_active_power)

# covert data to required date and time format
Date.Time <- strptime(paste(subsetplot3.data$Date, subsetplot3.data$Time, sep = " "), "%d/%m/%Y %H:%M:%S")
